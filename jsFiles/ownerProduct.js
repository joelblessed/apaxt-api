...
router.get("/sellerCatLader", async (req, res) => {
...
`;
        ...
        const { rows } = await query(queryText, [sellerId, language]);
        ...
        const shopName = rows.map((rw) => rw.owner)
        const shopId = rows.map((rw) => rw.owner_id)

        res.json({
            success: true,
            shop: [...new Set(shopName)],
            shop_id: [...new Set(shopId)],
...
router.get("/sellerCatSample", async (req, res) => {
    const sellerId = req.query.sellerId;
...
`;
        ...
        const { rows } = await query(groupedQuery, [language, sellerId]);
