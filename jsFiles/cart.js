    ...
    try {
        ...
        const cartItems = await query(
            `SELECT 
                ...
                ci.price_at_added,
                ci.discount_at_added,
                (ci.price_at_added - ci.discount_at_added) as final_price,
                ci.quantity,
                ci.metadata
             FROM cart_items ci
             JOIN user_products up ON ci.user_product_id = up.id
...
router.post("/cart", verifyToken, async (req, res) => {
    try {
        const { user_product_id, quantity, price_at_added, discount_at_added, metadata } = req.body; // Ensure quantity is handled
        if (!user_product_id) {
        ...
        const updatedCart = await query(
            `SELECT ci.id, p.id as product_id 
             FROM cart_items ci
